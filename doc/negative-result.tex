...
method --- the subexponential $\omega$-guided subset-product reduction of
Alford, Grantham, Hayman, and Shallue (AGHS) --- requires pool primes with
high \emph{identity density} on the group components, which we show is
unobtainable here: under the double Carmichael + Lucas--Carmichael condition,
harvestable pools have identity density $\approx 0.003$--$0.008$ (about
$100\times$ below AGHS pools), while the divisor-paradigm harvest that would
supply AGHS-like density yields an essentially empty pool ($0$--$2$ primes)
at all feasible scales. This quantified density gap is the negative result.
We emphasize scope: this is not a disproof of Agrawal's conjecture, nor a
proof that no counterexample can be constructed; one escape --- the full AGHS
algorithm run on low-density pools --- remains untested and is being pursued
separately.
\end{abstract}
...
\item[(ii)] \textbf{Identification and quantification of a solver-stage
obstruction} (\S\ref{sec:solver}). The subset-product problem splits into an
easy $2$-part ($\approx 16\%$ of demand bits, solved exactly at full scale)
and a dominant odd part ($\approx 84\%$) on which generic exact and heuristic
solvers cap two to three orders of magnitude short. The scalable method ---
the AGHS $\omega$-guided reduction \cite{AGHS}, which built a Carmichael
number with $10{,}333{,}229{,}505$ prime factors --- needs pools of high
\emph{identity density} ($\approx 0.3$--$0.5$). We measure that the double
Carmichael + Lucas--Carmichael condition forces a dichotomy: harvestable
pools have density $\approx 0.003$--$0.008$, and the divisor-paradigm
harvest that would achieve AGHS-like density is empty ($0$--$2$ primes) at
every feasible scale. The pipeline is blocked at the solver by this
quantified \emph{identity-density gap}.
...
harvest followed by subset-product solve, with the known solver families ---
is blocked at the solver stage, and it locates the blockage precisely (an
identity-density gap of about two orders of magnitude). One escape is
identified and untested; see \S\ref{sec:scope}, which should be read as part
of the result, not as a disclaimer.
...
\textbf{S1}--\textbf{S6} (S1 partition/coloring optimizer; \textbf{S2}
...
structure (H8) that the double condition makes unharvestable.
...
\subsection{The identity-density gap (the core negative result)}
\label{sec:gap}
...
\begin{finding}[The identity-density gap --- measured]\label{find:gap}
Under the double Carmichael + Lucas--Carmichael condition:
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
pool & identity density & pool size \\
\midrule
AGHS-style ($p-1 \mid \Lambda$, single condition) & $\approx 0.3$--$0.5$ &
large (their regime) \\
partition-first H7 (harvestable) & $\approx 0.003$--$0.008$ &
$\approx 10^5$ (over-provisioned) \\
divisor-paradigm H8 (AGHS density) & $\approx 0.07$--$0.18$ where nonempty &
$0$--$2$ (unharvestable) \\
\bottomrule
\end{tabular}
\end{center}
One can have a harvestable pool \emph{or} AGHS-like identity density ---
not both. The scalable solver family is therefore not straightforwardly
applicable, and no other family in the measured frontier
(Table~\ref{tab:frontier}) reaches the required dimension. That is the
obstruction.
\end{finding}

This is the punchline of the paper. The supply half of the pipeline is
solved and over-provisioned (\S\ref{sec:supply}); the solve half is blocked,
not by an absence of solvers, but by a quantified mismatch --- roughly two
orders of magnitude in identity density --- between what the scalable
solver needs and what the double condition can supply.

\section{Scope, caveats, and the untested escape}\label{sec:scope}
...
$\to$ subset-product solve) with the \emph{known solver families}
(linear-algebraic, annealing, meet-in-the-middle, and $\omega$-guided
reduction as characterized by its identity-density requirement). Other
constructions, and other solvers, are unconstrained by our measurements.
...
\item \textbf{The one open escape: the full AGHS solver on low-density
pools.} Whether the complete AGHS algorithm --- two potentials, birthday
step, non-uniformity exploitation --- tolerates the $\approx 0.003$--$0.008$
identity density of harvestable H7 pools is \emph{untested}. The
repository's $\omega$-reducer (\texttt{a8\_omega\_solver.py}) is a first
strict top-down version that solves only the easy regime ($r \le 20$
components at high identity density) and is too weak to settle the
question. This escape is being pursued separately from this write-up; a
positive outcome there would reopen the pipeline and supersede the negative
conclusion for route H7\,$\to$\,S6.
\item \textbf{Other unexhausted avenues.} A solver purpose-built for
low-density many-prime pools; a different AKS modulus $r$ (the group and
congruence classes change with $r$, and some $r \neq 5$ may admit denser
pools or a friendlier group); and the admissible-cell question of
\cite[Q.~6.2]{WP} --- the boost obstruction
\cite[Prop.~6.1]{WP} shows the mod-$80$ cell is unreachable by the
twin-smooth boosting family $2x^n - 1$ \cite{BCCEMNS23,Sterner23}, but
whether \emph{other} local-condition cells admit the L--P mechanism (and are
reachable by structurally side-separated families) is open, and a positive
answer would restructure the harvest entirely.
\end{enumerate}
...
\item \textbf{A solver-frontier map} for two-sided $0/1$ subset-product
problems (Table~\ref{tab:frontier}), with a synthetic-instance regression
harness (planted solutions, tunable identity density $\rho$) against which
any new solver --- in particular a full AGHS implementation --- can be
tested down to the harvestable regime $\rho \approx 0.004$.
\item \textbf{The identity-density gap as a diagnostic.} For any
Carmichael-type constructive problem under a \emph{double} side condition,
the two measurable quantities --- harvest yield of divisor-structured pools,
and identity density of harvestable pools --- locate the pipeline on the
harvestable/solvable dichotomy of Finding~\ref{find:gap} before major
compute is spent.
\end{itemize}

\emph{Conclusion.} The Lenstra--Pomerance construction reduces cleanly to a
two-sided subset-product problem whose supply side is, contrary to initial
expectation, solved: partition-first harvesting over-provisions the pool by
one to two orders of magnitude. The pipeline is blocked at the solver stage
by a quantified identity-density gap: harvestable pools are $\approx
100\times$ below the density the scalable $\omega$-guided solver family
exploits, and the harvest that would supply that density is empty under the
double Carmichael + Lucas--Carmichael condition. One escape --- the full
AGHS algorithm on low-density pools --- remains open and under separate
investigation. Until it is settled, the honest status of the constructive
route to refuting Agrawal's conjecture via twin smooths is: \emph{supply
yes, solver no, and the wall is measured}.
...
\begin{tabular}{@{}lll@{}}
\toprule
pool structure & identity density & harvestable under double condition? \\
\midrule
AGHS ($p - 1 \mid \Lambda$, highly composite $\Lambda$) &
$\approx 0.3$--$0.5$ & no (Table~\ref{tab:n1}: pool $0$--$2$) \\
H7 ($p - 1 = 2m$, $m$ a product of $j \approx 4$ medium primes) &
$\approx 0.003$--$0.008$ & yes (pool $\approx 10^5$) \\
\bottomrule
\end{tabular}
\caption{The identity-density gap (Finding~\ref{find:gap}): a
$\approx 100\times$ mismatch between what the scalable $\omega$-solver
exploits and what the double condition can supply. FINDINGS ``N1'',
subsection ``The identity-density gap''.}
\label{tab:gap}
\end{table}
